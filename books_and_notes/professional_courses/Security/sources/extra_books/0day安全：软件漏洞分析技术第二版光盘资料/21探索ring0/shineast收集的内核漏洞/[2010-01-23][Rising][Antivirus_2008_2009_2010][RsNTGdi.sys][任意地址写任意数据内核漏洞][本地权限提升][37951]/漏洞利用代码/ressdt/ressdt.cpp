...
void AddCallGate()
{
	...
	__asm
	{
		...
		mov dword ptr[CallGateData+2],0x0ec0003e8 //段选择子置为0x3e8
		//构造一个段选择子
		mov dword ptr[CallGateData+8],0x0000ffff  //段界限0xffff
		mov dword ptr[CallGateData+12],0x00cf9a00 
		xor eax,eax

LoopWrite:
		//将调用门和段选择子依次写入GDT的0x3e0和0x3e8偏移处
