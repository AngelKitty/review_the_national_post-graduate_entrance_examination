...
//////////////////////////////////failwest/////////////////////
int
s_add_binary(unsigned char * hex_stream)
{
  int i=0,k=0;
  ...
  while (!done)
	...
	null terminated. So don't do that.
      */
      if ( (hex_stream[i]=='0' && hex_stream[i+1]=='x') ||
	   (hex_stream[i]=='\\' && hex_stream[i+1]=='x') )
	{
	  i+=2;
	  continue;
	}
      ...
      if (!isxdigit(hex_stream[i]))
	{
      ...
      /*handle a hex digit*/
      if (isdigit(hex_stream[i]))
	currentnibble=hex_stream[i]-'0';
      else
	currentnibble=toupper(hex_stream[i])-'a'+10;
  ...
  //s_fuzzstring[i]=strdup(newfuzzstring);
  s_fuzzstring[k]=outarray;
  s_fuzzstring[k+1]=NULL;
