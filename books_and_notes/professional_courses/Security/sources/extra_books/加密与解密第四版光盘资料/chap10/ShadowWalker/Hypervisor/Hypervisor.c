    ...
    PGUEST_REGS GuestReg )
{
    ...
    else if ( GuestReg->RegEax == VMCALL_INIT_SPLIT )
    {
        //
        // 初始化TLB分割
        //
        ...
        Init_Split( CpuInfo->EptInfo, ( PTLB_TRANSLATION )GuestReg->RegEbx,
                    GuestReg->RegEcx );
    }
    else if ( GuestReg->RegEax == VMCALL_END_SPLIT )
    {
        //
        // 结束进程分割
        //
        End_Split( CpuInfo->EptInfo, ( PTLB_TRANSLATION )GuestReg->RegEbx,
                   GuestReg->RegEcx );
    }
...
//===========================================================================
    ...
    //填写基于针脚的虚拟机执行控制（Pin-based VM-execution controls）
    _WriteVMCS( PIN_BASED_VM_EXEC_CONTROL,
                VmxAdjustControls( 0, MSR_IA32_VMX_PINBASED_CTLS ) );

    //填写基于处理器的虚拟机执行控制（Primary processor-based VM-execution controls）
    //控制下面的异常位图、DR寄存器访问或者MSR访问是否产生#VMExit等
...
/*	Interceptions |=  CPU_BASED_MOV_DR_EXITING;		//DR寄存器访问 */
    ...
    //填写I/O bitmap（I/O位图），在这里可以拦截60号端口，也就是键盘输入
    ...
    //填写时间戳计数器偏移（Time-Stamp Counter Offset）
    _WriteVMCS( TSC_OFFSET, 0 );
    _WriteVMCS( TSC_OFFSET_HIGH, 0 );

    //填写虚拟机/Hypervisor屏蔽和CR0/CR4访问隐藏设置，没有用
    //填写CR3访问控制
    ...
    //
    //填写MSR位图地址（MSR Bitmap Address）
    //通过对MSR Bitmap填1来拦截需要拦截的MSR寄存器访问；当前填写置0，表示不需要拦截
    //
    ...
    //执行体VMCS指针(Executive-VMCS Pointer ), 没有用

//===========================================================================
    //
    //配置EPT指针( Extended Page Table Pointer )
    //
    ...
    //虚拟机标识符(Virtual Processor Identifier, VPID), 没有用
//===========================================================================
    //
    // 4.VMEntry行为控制域( VM-Entry Control Fields )
    //

    //VMEntry行为控制域（VM-Entry Control Fields）
