...
BOOL CheckSerial(HWND hWnd) 
{
	...
	for (i=0;i<4;i++)
	{
		sscanf((szSerial+i*4),"%04X",(wSN+i+1));
	}
	sha1_init(&sh);
	...
	memcpy(sztemp,(szHash+16),4);
	memcpy((sztemp+4),&dwVolumeSn,4);
	key(wiDeaKey,wSubKey);
	cip(wSN,wCipherText,wSubKey);
	memcpy(szBuffer,(wCipherText+1),8);
	
	if (memcmp(sztemp,szBuffer,8)==0)
	{
